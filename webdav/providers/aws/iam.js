// IAM provider — routes through the tube
...
async function getRoles() {
  return tubeRequest("aws/list-iam-roles", { filter: IAM_ROLE_FILTER });
}

async function getRoleDetail(roleName) {
  return tubeRequest("aws/get-iam-role", { roleName });
}

async function getUsers() {
  return tubeRequest("aws/list-iam-users");
}

async function getUserDetail(userName) {
  return tubeRequest("aws/get-iam-user", { userName });
}
...
export async function get(url, basePath) {
  ...
  if (trustMatch) {
    ...
    return { handled: true, content: JSON.stringify(detail.trust || {}, null, 2), contentType: "application/json" };
  }
